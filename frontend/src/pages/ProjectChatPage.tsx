...
export default function ProjectChatPage() {
    ...
    const {
      ...
      loadMoreMessages,
      hasMore,
    } = useProjectChat(parsedProjectId)

    const messagesEndRef = useRef<HTMLDivElement | null>(null)
    useEffect(() => {
        const loadProject = async () => {
            try {
            const data = await projectsApi.getById(parsedProjectId)
            setProject(data)
            } catch (error) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-lg font-semibold">
                  {`Чат проекта: ${project?.title}` || 'Чат проекта'}
                </h2>
                {project && (
                  <p className="text-xs text-muted-foreground">
                    {`Владелец: ${project.owner?.full_name}` || 'Неизвестный владелец'}
                  </p>
                )}
            </div>
          </div>
                  ...
                  {!isMe && msg.sender && (
                    <div className="flex items-center gap-2 mb-1">
                      <div className="h-6 w-6 rounded-full overflow-hidden bg-primary/20">
                        <AvatarImage
                          fileId={msg.sender.avatar_file_id}
                          fallback={msg.sender.full_name?.[0]?.toUpperCase() || '?'}
                          className="text-xs"
                        />
                      </div>

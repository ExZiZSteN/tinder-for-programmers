...
class SwipeRepository(BaseRepository[Swipe]):
    ...
    async def get_by_id_with_project(self, swipe_id: int) -> Swipe | None:
        result = await self.db.execute(
            ...
            selectinload(Swipe.project).selectinload(Project.owner),
            selectinload(Swipe.user).selectinload(User.user_skills).selectinload(UserSkill.skill),
        )
    ...
    async def get_inbox_by_owner(self, owner_id: int) -> Sequence[Swipe]:
        ...
        result = await self.db.execute(
            ...
            .where(
                Swipe.project_id.in_(project_ids),
                Swipe.status == SwipeStatus.PENDING,
            )
            .order_by(Swipe.created_at.desc())
        )
        return result.scalars().all()
